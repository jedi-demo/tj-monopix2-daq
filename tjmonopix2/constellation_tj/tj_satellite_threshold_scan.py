...
#class TJMonopix2(Satellite):
    def __init__(self, *args, **kwargs):
        ...
        self.threshold_scan = None
        self.thread_scan = None

    def do_initializing(self, config: Configuration) -> str:
        try:
            self.threshold_scan.close()
        except AttributeError:
            pass
        self._load_config(config)
        self.threshold_scan = ThresholdScan(scan_config=self.scan_configuration, bench_config=self.bench_conf)
        self.threshold_scan.init()
        return "initializing done"

    def do_launching(self) -> str:
        self.threshold_scan._init_environment()
        self.threshold_scan._init_hardware(force=False)
        self.threshold_scan.initialized = True
        self.threshold_scan.configure()
    ...
    def do_starting(self, run_identifier: str) -> str:
        ...
        self.threshold_scan._init_files()
        if hasattr(self.threshold_scan, "stop_scan"):
            self.threshold_scan.stop_scan.clear()
        self.thread_scan = threading.Thread(target=self.threshold_scan.scan)
